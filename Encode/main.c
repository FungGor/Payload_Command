...
#define STM32MCP_TORQUE_CONFIG_PAYLOAD_LENGTH        0x08
#define STM32MCP_TORQUE_RAMP_CONFIG_PAYLOAD_LENGTH   0x0A
...
void STM32MCP_TorqueConfiguration(uint16_t allowableSpeed, uint16_t IQValue)
{
   ...
   printf("Speed is : %u\n", SPEED);
   uint16_t IQ_Current = txFrame[6] + (txFrame[7] << 8) + (txFrame[9] <<16) + (txFrame[9] << 24);
...
void STM32MCP_TorqueRampConfiguration(uint16_t torqueIQ, uint16_t allowableSpeed, uint16_t rampRate)
{
   ...
   printf("%X\n", txFrame[11]);
   printf("%X\n", txFrame[12]);
   print_payload(txFrame, 13);
   printf("\n");
   uint16_t Current = txFrame[2] + (txFrame[3] << 8) + (txFrame[4] << 16) + (txFrame[5] << 24);
   printf("Maximum Current Allowed (s16): %u\n",Current);
   uint16_t RPM = txFrame[6] + (txFrame[7] << 8) + (txFrame[8] << 16) + (txFrame[9] << 24);
   printf("Maximum Speed Allowed : %u\n", RPM);
   uint16_t TIME = txFrame[10] + (txFrame[11] << 8);
